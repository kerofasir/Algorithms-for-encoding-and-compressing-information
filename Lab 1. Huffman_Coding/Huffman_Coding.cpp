...
int main() {
    ...
    if (Mode == 1) {
        ...
        if (Check_Files_Equal("text.txt", "decoded.txt")) printf("\nПроверка: Исходный файл text.txt совпадает с декодированным результатом в файле decoded.txt\n");
        else printf("\nПроверка: Исходный файл text.txt не совпадает с декодированным результатом в файле decoded.txt\n");
    }
    else {
