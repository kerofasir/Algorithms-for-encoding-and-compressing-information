...
void Encode_File(Node *Root); // Кодирует содержимое файла test.txt и сохраняет результат в encoded.txt
...
void Encode_File(Node *Root) {
    FILE *File_Input = fopen("test.txt", "rb");
...
void Decode_File(Node *Root) {
    ...
    int Total_Symbols = 0; //  счётчик длины исходного файла
    for (int i = 0; i < MAX; i++) {
        Total_Symbols += Frequency_Table[i];
    }
    int C;
    unsigned char Byte;
    Node *P = Root;
    int Decoded = 0; // вспомогательный счётчик
    while ((C = fgetc(File_Input)) != EOF && Decoded < Total_Symbols) {
        Byte = (unsigned char)C;
        for (int i = 7; i >= 0 && Decoded < Total_Symbols; i--) {
            int Bit = (Byte >> i) & 1;
            if (Bit) P = P->Right;
            else P = P->Left;
            if (!P->Left && !P->Right) {
...
int main() {
    ...
    if (Mode == 1) {
        FILE *File_Input = fopen("test.txt", "rb");
        if (!File_Input) {
            printf("Ошибка: Исходный файл test.txt не найден\n");
            return 1;
        }
        int C;
        while ((C = fgetc(File_Input)) != EOF) {
        ...
        if (Check_Files_Equal("test.txt", "decoded.txt")) printf("Проверка: Исходный файл test.txt совпадает с декодированным результатом в файле decoded.txt\n");
        else printf("Проверка: Исходный файл test.txt не совпадает с декодированным результатом в файле decoded.txt\n");
    }
